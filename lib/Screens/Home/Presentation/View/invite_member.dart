...
class InviteMembersScreen extends GetView<InviteMemberController> {
            ...
            actions: [
              if (!kIsWeb)
                TextButton(
                  ...
                  child: Text("Custom", style: BalooStyles.balooregularTextStyle(color: appColorYellow)),
                ).paddingOnly(left: 12),
            ],
          ),
          body: SafeArea(
                              ...
                              if (kIsWeb)
                                ...
                                SizedBox(
                                  ...
                                  child: ListView.builder(
                                    shrinkWrap: true,
                                    itemCount: c.filteredContacts.length,
                                    itemBuilder: (_, index) {
                                      final contact = c.filteredContacts[index];
                                      // show first normalized phone (10-digit)
                                      final firstPhone = contact.phones.isNotEmpty
                                          ? c.normalizePhone(contact.phones.first.number)
                                          : '';

                                      // true if any phone/email of contact already invited
                                      final isDisabled = _contactIsDisabled(contact, c);

                                      // keep invited ones checked by default (selectedContacts me seed ho chuka hai)
                                      final isSelected = c.selectedContacts.contains(contact);
                                      ...
                                      return CheckboxListTile(
                                        ...
                                        onChanged: isDisabled
                                            ...
                                            : (val) {
                                          final phoneNow = contact.phones.isNotEmpty
                                              ? c.normalizePhone(contact.phones.first.number)
                                              : '';
                                          if (val == true) {
                                            c.selectedContacts.add(contact);
                                            if (phoneNow.isNotEmpty &&
                                                !c.selectedInvitesContacts.contains(phoneNow)) {
                                              c.selectedInvitesContacts.add(phoneNow);
                                            }
                                            c.selectedUser.add(InviteUser(
                                              name: contact.displayName,
                                              mobile: phoneNow,
                                              isSelected: true,
                                            ));
                                          } else {
                                            c.selectedContacts.remove(contact);
                                            c.selectedInvitesContacts.remove(phoneNow);
                                ...
                                children: [
                                  Expanded(
                                    child: GradientButton(
                                      name: "Next",
                                      gradient: c.selectedUser.isEmpty
  ...
  bool _contactIsDisabled(Contact contact, InviteMemberController c) {
    for (final p in contact.phones) {
      ...
      if (id.isNotEmpty && c.disabledContactIdentifiers.contains(id)) return true;
    }
